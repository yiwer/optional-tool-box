...
> 状态：P1 已实施（分支 worktree-feat+auth-enhancement-p1）｜原设计稿 v0.1 裁定 R1-R5｜日期：2026-07-10
...
| 3 | Spring 默认 principal 名取 `sub`（UUID），业务日志/审计想要的是 `preferred_username`；且 Security Filter 层的 401/403 绕过 MVC 全局异常处理器，默认响应体与 facility `BaseResponse` 风格割裂 | principal claim 可配、默认 `preferred_username`（缺失回落 `sub`）；自带 EntryPoint/DeniedHandler 写 `BaseResponse` JSON（§4.3）。**实施修正（Task 6 实证）**：反向场景同真——`@PreAuthorize` 在 controller 方法内抛出的 `AccessDeniedException` 会被 facility 全局兜底 `@ExceptionHandler(Exception.class)` 吃成 200/500，模块以高优先级 `SecurityExceptionAdvice` 接管（§4.3） |
...
- **R6 fail-open 探测机制（2026-07-10 用户拍板，P2 B 项裁定轮）**：独立轻量 AutoConfiguration（`ToolboxAuthFailOpenDetector`，`AutoConfiguration.imports` 注册第二行），`@ConditionalOnMissingClass` 与主装配 L1 恰成反相；否决 EnvironmentPostProcessor（需 spring.factories 例外注册 + DeferredLog，时机早于需求）与 ApplicationListener（作 bean 注册塌缩回本方案，factories 注册同前者弊）。见 §5.5。
- **R7 探测粒度（2026-07-10 用户拍板）**：`toolbox.auth.enabled=false` 时静默——显式退出是 README 明示的合法出口，不制造噪音；以 `@ConditionalOnProperty(matchIfMissing=true)` 复用 L2 条件语义实现。无任何 `toolbox.auth.*` 键亦静默（无使用意图不骚扰）。
- **R8 jwks_unavailable 日志面（2026-07-10 用户拍板）**：观测码拆分顺带在取键失败分支打 WARN 单条——IdP 故障仅 debug 级对运维不可见；WARN 消息为 Nimbus 网络异常内容，无 token 成分，防探测原则不破。见 §4.3/§5.4。
...
│   └── SecurityExceptionAdvice.java# 实施修正（Task 6 实证补件）：方法级安全异常的 MVC 层出口（§4.3）
└── autoconfigure/
...
- **实施修正（Task 5 裁定，收窄原表述）**：EntryPoint/Filter 实现类不引 spring-web（只依赖 `jakarta.servlet` + facility `JsonUtil`）；`SecurityExceptionAdvice`（见下）是 MVC 组件，依赖 spring-web 的 `@RestControllerAdvice`/`@ExceptionHandler` 注解（optional 编译面依赖，非运行时强制）。
- 在 chain 中双挂：`oauth2ResourceServer(...)` 配置器内（bearer 路径）与 `exceptionHandling(...)`（非 bearer 路径，如匿名访问受保护端点），保证 401 形态处处一致。

**jwks_unavailable 观测码（P2 裁定 R8，2026-07-10）**：取键失败（KC 不可达）与 token 无效在 401 `description` 上区分——`KeycloakJwtDecoderFactory` 归一化时对"非 Bad 的 JwtException"改抛 `JwksUnavailableException`（jwt 包，`BadJwtException` 子类，401 非 500 语义因家族不变而保持），EntryPoint 沿 `getCause()` 链探测该类型（带深度上限防循环）⇒ `description=jwks_unavailable`，其余路径仍 `invalid_token`。`WWW-Authenticate` 头恒为标准码 `invalid_token`（RFC 6750 只注册三个错误码，自定义码不进协议头）；`SecurityExceptionAdvice` 委托既有 EntryPoint，MVC 路径自动同型。取键失败分支同处 WARN 单条（R8）。

**SessionUserBridgeFilter**（普通 jakarta `Filter`（非 `OncePerRequestFilter`，见类 Javadoc），仅 `bridge.session-user.enabled=true` 时加入 chain）：
...
**SecurityExceptionAdvice（实施修正，Task 6 全链矩阵实证补件）**：`@PreAuthorize` 拒绝在 controller 方法调用内部抛出的 `AccessDeniedException` 走 MVC 异常解析，绕开链上 `ExceptionTranslationFilter`、也绕开上述 EntryPoint/DeniedHandler 的双挂路径（§2.2 坑 3 反向场景）；不接管会被 facility 全局兜底 `@ExceptionHandler(Exception.class)`（`LOWEST_PRECEDENCE`）吃成 200/500。本组件是 MVC 层出口（`@RestControllerAdvice`），以 `@Order(Ordered.HIGHEST_PRECEDENCE + 1000)` 抢在全局兜底之前生效，同时给消费方留出更高优先级覆盖的空间；只接管两类安全异常——`AccessDeniedException`（→403）与 `AuthenticationException`（→401）——委托既有 `AuthAccessDeniedHandler`/`AuthEntryPoint` 写出同形态响应，其余异常仍归 facility 处理。
...
- **fail-fast 只对"真配置错误"**：issuer/jwks 端点本地派生（§4.2），启动期**不发任何网络请求**；JWKS 首次取用在首个 bearer 请求（Nimbus 缓存 + 未知 kid 自动刷新）。KC 暂时不可达 ⇒ 服务照常启动、bearer 请求 401（P2 裁定 R8 已拆观测码：取键失败经 `JwksUnavailableException`（BadJwtException 子类）归一化保 401 非 500，401 `description=jwks_unavailable` 与 `invalid_token` 区分并打 WARN，见 §4.3），**不**让 IdP 故障联动压垮服务启动。时钟偏移校验默认 60s（`clock-skew` 可配）；签名算法默认 `RS256`（`jws-algorithms` 可配列表，KC realm 可改 ES256 等）。

### 5.5 运行期 fail-open 探测（P2 裁定 R6/R7，2026-07-10）

L1 缺引擎类的退出态是 **fail-open**（README 警示段）：依赖重构时 starter 被意外 exclude，应用静默裸奔。探测器 `ToolboxAuthFailOpenDetector` 为独立轻量 AutoConfiguration（`AutoConfiguration.imports` 第二行）——主装配被 `@ConditionalOnClass` 挡住时它恰好活着：

- **条件栈**：`@ConditionalOnMissingClass("org.springframework.security.oauth2.jwt.JwtDecoder")`（L1 反相）+ `@ConditionalOnWebApplication(SERVLET)`（非 web 应用无裸奔面）+ `@ConditionalOnProperty(prefix="toolbox.auth", name="enabled", havingValue="true", matchIfMissing=true)`（R7：显式关停静默，复用 L2 条件语义）。
- **行为**：`@Bean` 方法注入 `Environment`，扫 `EnumerablePropertySource` 的 `toolbox.auth.` 前缀键；有键 ⇒ `LogUtil.warn` 单条（列出发现的键 + 行动指引：引 `spring-boot-starter-oauth2-resource-server`，或确认弃用并设 `toolbox.auth.enabled=false`）；无键 ⇒ 静默（R7）。产出探测结果 bean（嵌套 record，含发现的键列表）供装配矩阵断言在场性，不只靠日志断言。
- **边界**：只探测"缺引擎类"这一种 fail-open 退出态；`enabled=false` / `security-chain.enabled=false` 两种退出态落 Boot 默认链（fail-closed，README 已辨析），非探测对象。README fail-open 警示段实施时同步呼应探测器。
